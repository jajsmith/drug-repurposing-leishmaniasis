...
def load_homo_vs(path="./data", n=10):
    print("Loading Leishmania Data")
    df = pd.read_csv(path, sep="\t").sample(replace = False).reset_index(drop = True)
    return df['Target Sequence'].values, df['SMILES'].values, list(map(str, df['TargetID'].values)), list(map(str, df['SMILESID'].values))

# if __name__ == "__main__":
#     start = time()
#     # virtual_screening(*load_homo_vs("/global/home/hpc4763/repo/DeepPurpose/data/Homo/Homo_offtarget_Kd.tsv", 1778), save_dir=f"offtarget_effect_1")
#     virtual_screening(*load_homo_vs("rcsb.csv", 2500862), save_dir=f"rcsb")
#     # virtual_screening(*load_homo_vs("google.csv", 2679495), save_dir=f"google")
#     # virtual_screening(*load_homo_vs("homokd_similes445642.tsv", 1778), save_dir=f"offtarget_effect_6AZ1_445642")
#     print(time() - start)
...
prot = pd.read_csv('final/r_seq.csv').to_numpy()
...
for seqID, seq in prot:
    for smileID, smile in drug:
        ...
        tid.append(seqID)
        sid.append(smileID)
virtual_screening(ts, sm, tid, sid, save_dir='rcsb')
...
for seqID, seq in prot:
    for smileID, smile in drug:
        ...
        tid.append(seqID)
        sid.append(smileID)
virtual_screening(ts, sm, tid, sid, save_dir='google')
